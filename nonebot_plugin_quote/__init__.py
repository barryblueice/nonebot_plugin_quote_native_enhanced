...
async def save_img_handle(bot: Bot, event: GroupMessageEvent, state: T_State, Session: EventSession):
    ...
    if event.reply:
        ...
        if match:
            ...
            await make_record.finish(MessageSegment.text("未检测到图片，请回复所需上传的图片消息来上传语录"), at_sender = True)
    else:
        await make_record.finish(MessageSegment.text("请回复所需上传的图片消息来上传语录"), at_sender = True)
...
async def record_help_handle(bot: Bot, event: GroupMessageEvent, state: T_State, Session: EventSession):
    ...
    await record_help.finish(MessageSegment.text(msg), at_sender = True)
...
async def delete_record_handle(bot: Bot, event: Event, state: T_State, Session: EventSession):
    ...
    await delete_record.finish(message=MessageSegment.text(msg), at_sender = True)
...
async def alltag_handle(bot: Bot, event: GroupMessageEvent, state: T_State, Session: EventSession):
    ...
    await alltag.finish(MessageSegment.text(msg), at_sender = True)
...
async def addtag_handle(bot: Bot, event: GroupMessageEvent, state: T_State, Session: EventSession):
    ...
    await addtag.finish(message=MessageSegment.text(msg), at_sender = True)
...
async def deltag_handle(bot: Bot, event: GroupMessageEvent, state: T_State, Session: EventSession):
    ...
    await deltag.finish(message=MessageSegment.text(msg), at_sender = True)
...
rumor_quote = on_regex(pattern="^{}rumor.*".format(re.escape(plugin_config.quote_startcmd)), priority=3, block=True)

@rumor_quote.handle()
async def rumor_quote_hanle(bot: Bot, event: GroupMessageEvent, state: T_State, Session: EventSession):
